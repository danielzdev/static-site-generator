...
class TestSplitNodesDelimiter(unittest.TestCase):
    ...
    def test_multiple_textnodes_mixed_delimiters(self):
        ...
        self.assertEqual(result[0].text, "Middle has ")
        self.assertEqual(result[0].text_type, TextType.TEXT)
        self.assertEqual(result[1].text, "*bold*")
        self.assertEqual(result[1].text_type, TextType.BOLD)

        self.assertEqual(result[2].text, " text, neat.")
        self.assertEqual(result[2].text_type, TextType.TEXT)

        self.assertEqual(result[3].text, "Some ")
        self.assertEqual(result[3].text_type, TextType.TEXT)
        self.assertEqual(result[4].text, "*MORE*")
        self.assertEqual(result[4].text_type, TextType.BOLD)

        self.assertEqual(result[5].text, " bold text :D")
